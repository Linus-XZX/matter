...
Also note that macOS release builds and all iOS simulator builds (`flutter build ios --simulator`) are broken on macOS 27 due to [this Flutter issue](https://github.com/flutter/flutter/issues/188461), which has been fixed with [this PR](https://github.com/flutter/flutter/pull/188625) but appears to not be in stable as of Flutter 3.44.6. Debug builds on macOS (`flutter build macos --debug`) are not affected. Non-simulator iOS builds are not tested as I do not have the correct provisioning setup yet.
