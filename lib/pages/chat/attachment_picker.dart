...
class _AttachmentPickerState extends State<AttachmentPicker> {
  ...
  Future<void> _sendMediaAssets(List<AssetEntity> assets) => _doSend(() async {
    for (final asset in assets) {
      await _sendSingleAsset(asset);
    }
  });
  ...
  Future<void> _sendEditedImage(Uint8List bytes) => _doSend(() async {
  ...
  Future<void> _sendFiles(List<XFile> files) => _doSend(() async {
  ...
  Future<void> _sendLocation(String body, String geoUri) => _doSend(
    () => rust.sendLocation(roomId: widget.roomId, body: body, geoUri: geoUri),
  );

  /// Wraps a send action with the loading flag, refresh, and the
  /// [MessageInput]-style completion callback.
  Future<void> _doSend(Future<void> Function() action) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          switch (_tab) {
            AttachmentTab.media => _MediaTabBody(
              ...
              onOpenEditor: (bytes) async {
                final edited = await Navigator.of(context).push<Uint8List>(
                  MaterialPageRoute(
                    ...
                    builder: (_) => ChatImageEditorPage(imageBytes: bytes),
                  ),
                );
                if (edited != null && mounted) await _sendEditedImage(edited);
              },
            ),
...
class _MediaTabBodyState extends State<_MediaTabBody> {
  ...
  Future<void> _fetchMore() async {
    ...
    if (_assets.length >= (await _album!.assetCountAsync)) return;
      ...
      children: [
        ...
        if (_selected.isNotEmpty)
          _SendBar(
            ...
            onSend: () => widget.onSendAssets(List.of(_selected)),
          ),
      ],
    );
  }
